...
export function checkoutCredentialFailures(workflow, relative) {
  ...
  for (const [index, line] of lines.entries()) {
    if (!/^-?\s*uses:\s+actions\/checkout@/.test(line.trim())) continue;
    ...
    for (const [offset, following] of lines.slice(index + 1).entries()) {
      ...
      if (followingIndentation === usesIndentation && following.trim() === "with:") withHeaders.push(index + offset + 1);
    }
    const entries = withHeaders.length === 1 ? directMappingLines(lines, withHeaders[0], usesIndentation) : [];
    const credentialEntries = entries.filter((entry) => /^(?:["']persist-credentials["']|persist-credentials)\s*:/.test(entry));
    const exactFalse = credentialEntries.length === 1 && /^(?:["']persist-credentials["']|persist-credentials)\s*:\s*false(?:\s+#.*)?$/.test(credentialEntries[0]);
...
function permissionFailures(lines, relative) {
  const failures = [];
  const headers = lines.map((line, index) => ({ index, indentation: indentation(line), line })).filter(({ line }) => /^permissions\s*:/.test(line.trim()));
  ...
  let exact = topLevel.length === 1 && topLevel[0].line.trim() === "permissions:";
  if (exact) {
    const entries = [];
